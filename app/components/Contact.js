...
const codeTokens = [
  { text: 'const',        x: '5%',  y: '12%', color: '#a78bfa', dur: 9,  delay: 0,   rotX: 15, rotY: 20 },
  { text: 'async/await',  x: '80%', y: '8%',  color: '#34d399', dur: 11, delay: 1.2, rotX: -10, rotY: 25 },
  { text: '() => {}',     x: '88%', y: '55%', color: '#60a5fa', dur: 8,  delay: 0.5, rotX: 20, rotY: -15 },
  { text: 'useState()',   x: '3%',  y: '60%', color: '#34d399', dur: 13, delay: 2,   rotX: -20, rotY: 10 },
  { text: '<Component/>', x: '70%', y: '80%', color: '#f472b6', dur: 10, delay: 0.8, rotX: 10, rotY: 30 },
  { text: 'npm install',  x: '20%', y: '85%', color: '#fbbf24', dur: 12, delay: 1.5, rotX: -15, rotY: -20 },
  { text: 'git commit',   x: '45%', y: '5%',  color: '#f87171', dur: 9,  delay: 3,   rotX: 25, rotY: 15 },
  { text: '.map()',       x: '55%', y: '90%', color: '#60a5fa', dur: 7,  delay: 0.3, rotX: -10, rotY: -25 },
  { text: 'return (',     x: '92%', y: '30%', color: '#a78bfa', dur: 14, delay: 2.5, rotX: 15, rotY: 20 },
  { text: '{ }',          x: '12%', y: '35%', color: '#34d399', dur: 6,  delay: 1,   rotX: -5, rotY: 35 },
]

const ThreeDBg = () => (
  <div className="absolute inset-0 overflow-hidden pointer-events-none" style={{ zIndex: 1, perspective: '600px' }}>
    {codeTokens.map((t, i) => (
      <motion.div
        key={i}
        ...
        style={{
          ...
          fontSize: '0.75rem',
          color: t.color,
          textShadow: `0 0 12px ${t.color}55, 0 0 24px ${t.color}22`,
          transformStyle: 'preserve-3d',
          transform: `rotateX(${t.rotX}deg) rotateY(${t.rotY}deg)`,
          opacity: 0.45,
        ...
        animate={{
          y: [0, -18, 0],
          rotateY: [t.rotY, t.rotY + 360],
          opacity: [0.35, 0.6, 0.35],
        }}
        transition={{
          y:       { duration: t.dur * 0.6, repeat: Infinity, ease: 'easeInOut', repeatType: 'reverse', delay: t.delay },
          rotateY: { duration: t.dur,       repeat: Infinity, ease: 'linear',    delay: t.delay },
          opacity: { duration: t.dur * 0.8, repeat: Infinity, ease: 'easeInOut', repeatType: 'reverse', delay: t.delay },
        }}
      >
        {/* glowing underline */}
        <span style={{ borderBottom: `1px solid ${t.color}44`, paddingBottom: 1 }}>{t.text}</span>
      </motion.div>
    ))}
  </div>
)
...
export default function Contact() {
  ...
  const contactInfo = [
    { icon: <Mail className="w-4 h-4" />, label: 'email', value: 'vaibhav34kashyap@gmail.com', link: 'mailto:vaibhav34kashyap@gmail.com' },
    { icon: <Phone className="w-4 h-4" />, label: 'phone', value: '+91 75055 36264', link: 'tel:+917505536264' },
    { icon: <MapPin className="w-4 h-4" />, label: 'location', value: 'Chandigarh, India', link: '#' },
    { icon: <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>, label: 'whatsapp', value: '+91 75055 36264', link: 'https://wa.me/917505536264' },
  ]
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-6 max-w-5xl mx-auto">
          ...
          >
            ...
            <div className="flex items-center gap-2 px-4 py-3 bg-gray-200 dark:bg-gray-800 border-b border-gray-300 dark:border-gray-700">
              ...
              <span className="ml-2 text-xs font-mono text-gray-500 dark:text-gray-400">vaibhav@portfolio ~ contact</span>
            </div>
